...
const details = (): IpluginDetails => ({
  ...
  description:
    'Compress lossless audio codecs (DTS-HD MA, TrueHD, FLAC, PCM) to lossy audio codecs.',
  style: {
    borderColor: '#6efefc',
  },
  tags: 'audio',
  ...
  inputs: [
    {
      ...
      tooltip:
        'Toggle whether DTS-HD MA audio streams will be compressed or not.',
    },
    {
...
const plugin = (args: IpluginInputArgs): IpluginOutputArgs => {
  ...
  for (let i = 0; i < args.variables.ffmpegCommand.streams.length; i += 1) {
    ...
    const compressDTSHDMA = Boolean(args.inputs.compressDTSHDMA);
    const compressTrueHD = Boolean(args.inputs.compressTrueHD);
    const compressFLAC = Boolean(args.inputs.compressFLAC);
    const compressPCM = Boolean(args.inputs.compressPCM);

    let processStream = compressDTSHDMA && stream.codec_name === 'dts' && stream.profile === 'DTS-HD MA';
